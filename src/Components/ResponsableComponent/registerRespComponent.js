...
function RegisterRespComponent() {
  ...
  const navigate = useNavigate();
  const API_BASE_URL="http://localhost:8080/api/responsable/registre";
  useEffect(() => {
    ...
    axios
      .post(API_BASE_URL,data ,
      {
        ...
        .then((response) => {
         ...
         .catch((error) => {
         console.error(error.response.data);
          if (error.response.data === "email exsist ") {
...
marginLeft:'110px'
 ...
 <label htmlFor="floatingSelect">Centre</label>
      ...
      <p className="text-center mb-0">Vous avez de compte? <Link to="/loginR">Connexion</Link></p>
      </form>
    </div>
  </div>
</div>
...
export default RegisterRespComponent;